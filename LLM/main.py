...
agent = create_agent(
    model=llm,
    tools=[ML_predict ],
    system_prompt="Ты авто-ассистент. Если нужно предсказать цену машины, используй ML_predict.Но сам не придумывай цену, а всегда вызывай ML_predict с правильными характеристиками машины. Если не хватает данных, попроси их у пользователя.Еще переобразуй на формат json чтобы потом скинуть в ML_predict"
)
